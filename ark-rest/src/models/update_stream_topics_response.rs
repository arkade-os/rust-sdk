/*
 * Ark API
 *
 * Combined Ark Service, Indexer, Admin, Signer Manager, and Wallet API
 *
 * The version of the OpenAPI document: 1.0.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::Deserialize;
use serde::Serialize;

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct UpdateStreamTopicsResponse {
    #[serde(rename = "allTopics", skip_serializing_if = "Option::is_none")]
    pub all_topics: Option<Vec<String>>,
    #[serde(rename = "topicsAdded", skip_serializing_if = "Option::is_none")]
    pub topics_added: Option<Vec<String>>,
    #[serde(rename = "topicsRemoved", skip_serializing_if = "Option::is_none")]
    pub topics_removed: Option<Vec<String>>,
}

impl UpdateStreamTopicsResponse {
    pub fn new() -> UpdateStreamTopicsResponse {
        UpdateStreamTopicsResponse {
            all_topics: None,
            topics_added: None,
            topics_removed: None,
        }
    }
}
