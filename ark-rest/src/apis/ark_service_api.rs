...
/// struct for typed errors of method [`ark_service_get_intent2`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ArkServiceGetIntent2Error {
    DefaultResponse(models::Status),
    UnknownValue(serde_json::Value),
}
...
pub async fn ark_service_get_intent2(
    configuration: &configuration::Configuration,
    get_intent_request: models::GetIntentRequest,
) -> Result<models::GetIntentResponse, Error<ArkServiceGetIntent2Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_get_intent_request = get_intent_request;
    ...
    req_builder = req_builder.json(&p_get_intent_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ArkServiceGetIntent2Error> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
