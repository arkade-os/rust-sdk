...
async fn main() -> Result<()> {
    ...
    let rest_client = ark_rest::Client::new(ark_server_url);

    let server_info = rest_client.get_info().await?;
    ...
    match &cli.command {
        Commands::Balance => {
            let virtual_tx_outpoints = {
                let spendable_vtxos = spendable_vtxos(&rest_client, &[vtxo], false).await?;
        ...
        Commands::SendToArkAddress { address, amount } => {
            ...
            let virtual_tx_outpoints = {
                let spendable_vtxos =
                    spendable_vtxos(&rest_client, std::slice::from_ref(&vtxo), false).await?;
                list_virtual_tx_outpoints(find_outpoints_fn, spendable_vtxos)?
            };
        ...
        Commands::Settle => {
            let virtual_tx_outpoints = {
                let spendable_vtxos =
                    spendable_vtxos(&rest_client, std::slice::from_ref(&vtxo), true).await?;
                list_virtual_tx_outpoints(find_outpoints_fn, spendable_vtxos)?
            };
            let boarding_outpoints =
                list_boarding_outpoints(find_outpoints_fn, &[boarding_output])?;
...
async fn spendable_vtxos(
    ...
    for vtxo in vtxos.iter() {
        ...
        let list = rest_client.list_vtxos(request).await?;
...
async fn settle(
    ...
    let (bip322_proof, intent_message) = proof_of_funds::make_bip322_signature(
        ...
        IntentMessageType::Delete,
    )?;
    let delete_intent_message = intent_message.clone().to_delete();
    if let Err(err) = rest_client
        .delete_intent(&delete_intent_message, &bip322_proof)
        .await
    {
        tracing::error!("Failed deleting intent: {:?}", err);
    }
    ...
    let mut event_stream = rest_client.get_event_stream(topics).await?;
    dbg!("2");
    ...
    if batch_started_event
        ...
        .any(|h| h == &hash)
    {
        rest_client.confirm_registration(intent_id.clone()).await?;
    } else {
        bail!(
...
async fn transaction_history(
    ...
    for vtxo in vtxos.iter() {
        let request = GetVtxosRequest::new_for_addresses(&[vtxo.to_ark_address()]);
        let vtxo_list = rest_client.list_vtxos(request).await?;
